import time
from flask import Flask, Response, jsonify, render_template, request, send_from_directory, redirect
...
def start_system():
    if not coordinator.camera_active:
        coordinator.start_camera(int(request.form["videoSource"]), float(request.form["scale"]))
